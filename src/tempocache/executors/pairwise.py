"""Pairwise executors for MatMul/BMM."""
...
class MatMulExecutor(ExecutorBase):
    """Grouped execution for torch.matmul-style binary ops."""
...
class BMMExecutor(ExecutorBase):
    """Grouped execution for torch.bmm-style [*,M,N] x [*,N,P] ops."""
    ...
    def execute(
        ...
        *,
        ...
        if full_idx.numel() > 0:
            a_full = a_win[:, full_idx]
            b_full = b_win[:, full_idx]
            a_flat = a_full.reshape(k * full_idx.numel(), *a_full.shape[2:])
            b_flat = b_full.reshape(k * full_idx.numel(), *b_full.shape[2:])
            y_flat = op(a_flat, b_flat)
        ...
        if reuse_idx.numel() > 0:
            if cache_state.cache is None:
                a_reuse = a_win[:, reuse_idx]
                b_reuse = b_win[:, reuse_idx]
                a_flat = a_reuse.reshape(k * reuse_idx.numel(), *a_reuse.shape[2:])
                b_flat = b_reuse.reshape(k * reuse_idx.numel(), *b_reuse.shape[2:])
                y_flat = op(a_flat, b_flat)
